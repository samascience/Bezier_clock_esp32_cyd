...
class Display(object):
    ...
    def __init__(self, spi, cs, dc, rst, width=240, height=320, rotation=0,
        ...
        self.reset()
        # Send ST7789 initialization commands (V3 USB-C boards)
        self.write_cmd(self.SWRESET)  # Software reset
        sleep(.15)
        self.write_cmd(self.SLPOUT)   # Exit sleep mode
        sleep(.15)
        self.write_cmd(self.PIXFMT, 0x55) # COLMOD: 16-bit color
        sleep(.01)
        self.write_cmd(self.MADCTL, self.rotation)  # Memory access ctrl
        self.write_cmd(self.INVOFF)   # Disable display inversion (correct for this panel)
        self.write_cmd(self.NORON)    # Normal display mode on
